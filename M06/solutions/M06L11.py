...
def get_filename_and_extension(filename: str) -> list[str]:
    if '.' in filename:
...
def print_operations(operations: list[RenameOperation]) -> None:
    print("ZOSTANĄ DOKONANE NASTĘPUJĄCE ZMIANY:")
    for op in operations:
        print(op)


def execute_operations(operations: list[RenameOperation]) -> None:
    for op in operations:
        op.execute()
        print("Zmieniono", op)
